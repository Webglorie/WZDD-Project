                        ...
                        <div class="card card-border-primary">
                            ...
                            <div class="card-body p-3 swim-lane swim-lane-card">
                                ...
                                @foreach($employeeSchedules as $employeeSchedule)
                                    @if ($employeeSchedule['category'] != null && $employeeSchedule['category']->id == $category->id)
                                        @php
                                            $assignedEmployees[] = $employeeSchedule['employee'];
                                        @endphp
                                    @endif
                                @endforeach
                                @if (count($assignedEmployees) > 0)
                                    @foreach($assignedEmployees as $assignedEmployee)
                                            ...
                                            <div class="card mb-3 bg-light task" draggable="true"
                                                 data-attendancename="{{ $assignedEmployee->name }}"
                                                 data-attendanceid="{{ $assignedEmployee->id }}"
                                                 data-catid="{{ $category->id }}">
                                                <div class="card-body p-3">

                                                    <p class="card-data employee-name">{{ $assignedEmployee->name }}</p>


                                                    <!-- Button trigger modal -->
                                                    <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#scheduleModal{{ $assignedEmployee->id }}">
                                                        Bekijk Rooster
                                                    </button>
                                                    <div class="d-flex align-items-center mt-2">
                                                        <button type="button" class="btn btn-primary btn-sm edit-btn" data-toggle="modal" data-target="#modal-{{ $assignedEmployee->id }}">
                                                            <i class="fas fa-edit"></i>
                                                        </button>
                                                        <span class="m-1"><i data-attendanceid="{{ $assignedEmployee->id }}" class="fa fa-trash remove-card"></i></span>
                                                    </div>
                                                </div>
                                            </div>
                                        <!-- Modal -->
